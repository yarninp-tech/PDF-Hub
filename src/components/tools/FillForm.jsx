...
export default function FillForm() {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: async ([file]) => {
      ...
      try {
        ...
        // Detect form fields using pdfjs
        const fields = []
  ...
  // Get page dimensions from pdfDoc
  ...
  const getFieldStyle = (field) => {
    ...
    const top = (pageDims.height - y2) * scaleY
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      downloadBytes(outBytes, `${getBaseName(pdfFile.name)}_filled.pdf`)
    } catch (err) {
    ...
    <div className="flex h-full">
      ...
      <div className="w-72 bg-white border-r border-gray-200 flex flex-col">
        <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
          ...
          <button
            onClick={() => { setPdfFile(null); setPdfDoc(null); setFormFields([]); setFormValues({}) }}
      ...
      <div className="flex-1 overflow-auto bg-gray-100 flex items-start justify-center p-4">
        <div className="relative bg-white shadow-xl" style={{ display: 'inline-block' }} ref={containerRef}>
            ...
            <div key={field.id} style={getFieldStyle(field)}>
              ...
              {field.type === 'Ch' && (
                <select
                  value={formValues[field.name] || ''}
                  onChange={e => setFormValues(v => ({ ...v, [field.name]: e.target.value }))}
                  className="w-full h-full text-xs bg-blue-50 border border-blue-300 rounded focus:outline-none"
                >
                  <option value="">Select...</option>
                  {field.options.map(opt => (
                    <option key={opt.exportValue} value={opt.exportValue}>{opt.displayValue}</option>
