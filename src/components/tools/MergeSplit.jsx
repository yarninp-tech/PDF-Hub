...
// mergeItem shape:
// { id: string, file: File, pageCount: number, thumbnailDoc: pdfjs doc, selectedPages: number[], expanded: boolean }
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">
            ...
            {item.pageCount > 0 && ' · ' + item.selectedPages.length + ' selected'}
          </p>
        </div>
        {item.pageCount > 0 && (
          <button
...
export default function MergeSplit({ pdfFile: globalFile, pdfDoc: globalDoc, pageCount: globalPageCount, onOpenFile }) {
  ...
  const { getRootProps: getMergeRootProps, getInputProps: getMergeInputProps, isDragActive: isMergeDragActive } = useDropzone({
    ...
    onDrop: async function(accepted) {
      var newItems = await Promise.all(accepted.map(async function(file) {
        var pageCount = 0
        var thumbnailDoc = null
        var selectedPages = []
        try {
          var result = await loadPDF(file)
          thumbnailDoc = result.pdfDoc
          pageCount = thumbnailDoc.numPages
          for (var i = 1; i <= pageCount; i++) selectedPages.push(i)
        } catch (_) {}
        return {
          id: file.name + '-' + Date.now() + '-' + Math.random(),
          file: file,
          pageCount: pageCount,
          thumbnailDoc: thumbnailDoc,
  ...
  function handleSelectedPagesChange(index, pages) {
    setMergeItems(function(prev) {
      return prev.map(function(item, i) {
        ...
        return { id: item.id, file: item.file, pageCount: item.pageCount, thumbnailDoc: item.thumbnailDoc, selectedPages: pages, expanded: item.expanded }
      })
    })
  }
  ...
  function handleToggleExpanded(index) {
    setMergeItems(function(prev) {
      return prev.map(function(item, i) {
        if (i !== index) return item
        return { id: item.id, file: item.file, pageCount: item.pageCount, thumbnailDoc: item.thumbnailDoc, selectedPages: item.selectedPages, expanded: !item.expanded }
      })
    })
  }
    ...
    try {
      ...
      if (splitMode === 'individual') {
        if (splitPageCount === 1) {
          ...
          var pages = await newDoc.copyPages(srcDoc, [0])
          newDoc.addPage(pages[0])
          var b = await newDoc.save()
          downloadBytes(b, baseName + '_page1.pdf')
        } else {
          var zip = new JSZip()
          for (var i = 0; i < splitPageCount; i++) {
            var nd = await PDFDocument.create()
            var pg = await nd.copyPages(srcDoc, [i])
            nd.addPage(pg[0])
            var pb = await nd.save()
            zip.file(baseName + '_page' + (i + 1) + '.pdf', pb)
          }
        ...
        var rp = await rd.copyPages(srcDoc, pageNums.map(function(p) { return p - 1 }))
        rp.forEach(function(p) { rd.addPage(p) })
        var rb = await rd.save()
        downloadBytes(rb, baseName + '_extracted.pdf')
      }
    } catch (err) {
      setSplitError(err.message)
    } finally {
    ...
    <div className="p-6 max-w-3xl mx-auto w-full">
      ...
      {tab === 'merge' && (
        <div className="space-y-4">
          ...
          <div
            {...getMergeRootProps()}
            className={'border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ' + (isMergeDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-blue-400 hover:bg-gray-50')}
          >
          ...
          {mergeItems.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm text-gray-500 font-medium">
                {mergeItems.length} file{mergeItems.length > 1 ? 's' : ''} selected · drag to reorder
              </p>
